...
uint8_t lang_select(uint8_t lang) { (void)lang; return 0; }
uint8_t lang_get_count() { return 1; }
uint16_t lang_get_code(uint8_t lang) { (void)lang; return LANG_CODE_EN; }
const char* lang_get_name_by_code(uint16_t code) { (void)code; return _n("English"); }
